...
#include "EightEngine\Graphics\DirectX11\Shaders\D3D11PixelShader.h"
...
class PixelShader : public D3D11PixelShader
{
public:
	PixelShader() { }
	~PixelShader() { }

	// Creating pixel shader from memory and creating batching group
		...
		const std::shared_ptr<RenderingDeviceContext> &pRenderingDeviceContext) override
	{
		...
		std::function<void(UINT)> functionCB = std::bind(
			&RenderingDeviceContext::BatchBindConstantBufferPS,
		...
		std::function<void(UINT)> functionSS = std::bind(
			&RenderingDeviceContext::BatchBindSamplerStatePS,
			pRenderingDeviceContext.get(), std::placeholders::_1);

		m_SamplerStateBatch.SetBindingFunction(functionSS);

		// Create pixel shader from specific API(e.g DirectX, Opengl etc.)
		return CreateApiSpecific(pShaderBytecode, byteCodeSize,
			pRenderingDevice);
	}

	// Binding shader, constant buffers and sampler states into pixel stage
		...
		pRenderingDeviceContext) override
	{
		...
		if (pRenderingDeviceContext->GetRenderingPipelineTable().TryBindShader(
			GetShaderAddress(), static_cast<UINT>(ShaderType::PIXEL_SHADER)))
		{

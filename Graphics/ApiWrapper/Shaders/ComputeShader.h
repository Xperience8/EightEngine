...
#include "EightEngine\Graphics\DirectX11\Shaders\D3D11ComputeShader.h"
...
class ComputeShader : public D3D11ComputeShader
{
public:
	ComputeShader() { }
	~ComputeShader() { }

	// Creating compute shader from memory and creating batching group
		...
		const std::shared_ptr<RenderingDeviceContext> &pRenderingDeviceContext) override
	{
		...
		std::function<void(UINT)> functionCB = std::bind(
			&RenderingDeviceContext::BatchBindConstantBufferCS,
		...
		std::function<void(UINT)> functionSS = std::bind(
			&RenderingDeviceContext::BatchBindSamplerStateCS,
			pRenderingDeviceContext.get(), std::placeholders::_1);

		m_SamplerStateBatch.SetBindingFunction(functionSS);

		// Create compute shader from specific API(e.g DirectX, Opengl etc.)
		return CreateApiSpecific(pShaderBytecode, byteCodeSize,
			pRenderingDevice);
	}

	// Binding shader, constant buffers and sampler states into compute stage
		...
		pRenderingDeviceContext) override
	{
		...
		if (pRenderingDeviceContext->GetRenderingPipelineTable().TryBindShader(
			GetShaderAddress(), static_cast<UINT>(ShaderType::COMPUTE_SHADER)))
		{

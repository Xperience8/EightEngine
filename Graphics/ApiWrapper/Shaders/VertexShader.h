...
#include "EightEngine\Graphics\DirectX11\Shaders\D3D11VertexShader.h"
...
class VertexShader : public D3D11VertexShader
{
public:
	VertexShader() { }
	~VertexShader() { }

	// Creating vertex shader from memory and creating batching group
		...
		const std::shared_ptr<RenderingDeviceContext> &pRenderingDeviceContext) override
	{
		...
		std::function<void(UINT)> functionCB = std::bind(
			&RenderingDeviceContext::BatchBindConstantBufferVS,
		...
		std::function<void(UINT)> functionSS = std::bind(
			&RenderingDeviceContext::BatchBindSamplerStateVS,
			pRenderingDeviceContext.get(), std::placeholders::_1);

		m_SamplerStateBatch.SetBindingFunction(functionSS);

		// Create vertex shader from specific API(e.g DirectX, Opengl etc.)
		...
		pRenderingDeviceContext) override
	{
		...
		if (pRenderingDeviceContext->GetRenderingPipelineTable().TryBindShader(
			GetShaderAddress(), static_cast<UINT>(ShaderType::VERTEX_SHADER)))
		{

...
#include "EightEngine\Graphics\DirectX11\Shaders\D3D11HullShader.h"
...
class HullShader : public D3D11HullShader
{
private:
...
public:
	...
	// Creating hull shader from memory and creating batching group
		...
		const std::shared_ptr<RenderingDeviceContext> &pRenderingDeviceContext) override
	{
		...
		std::function<void(UINT)> functionCB = std::bind(
			&RenderingDeviceContext::BatchBindConstantBufferHS,
		...
		std::function<void(UINT)> functionSS = std::bind(
			&RenderingDeviceContext::BatchBindSamplerStateHS,
			pRenderingDeviceContext.get(), std::placeholders::_1);

		m_SamplerStateBatch.SetBindingFunction(functionSS);

		// Create hull shader from specific API(e.g DirectX, Opengl etc.)
		return CreateApiSpecific(pShaderBytecode, byteCodeSize,
			pRenderingDevice);
	}

	// Binding shader, constant buffers and sampler states into hull stage
		...
		pRenderingDeviceContext) override
	{
		...
		if (pRenderingDeviceContext->GetRenderingPipelineTable().TryBindShader(
			GetShaderAddress(), static_cast<UINT>(ShaderType::HULL_SHADER)))
		{

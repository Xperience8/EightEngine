...
#include "EightEngine\Graphics\DirectX11\Shaders\D3D11GeometryShader.h"
...
class GeometryShader : public D3D11GeometryShader
{
public:
	GeometryShader() { }
	~GeometryShader() { }

	// Creating geometry shader from memory and creating batching group
		...
		const std::shared_ptr<RenderingDeviceContext> &pRenderingDeviceContext) override
	{
		...
		std::function<void(UINT)> functionCB = std::bind(
			&RenderingDeviceContext::BatchBindConstantBufferGS,
		...
		std::function<void(UINT)> functionSS = std::bind(
			&RenderingDeviceContext::BatchBindSamplerStateGS,
			pRenderingDeviceContext.get(), std::placeholders::_1);

		m_SamplerStateBatch.SetBindingFunction(functionSS);

		// Create geometry shader from specific API(e.g DirectX, Opengl etc.)
		return CreateApiSpecific(pShaderBytecode, byteCodeSize,
			pRenderingDevice);
	}

	// Binding shader, constant buffers and sampler states into geometry stage
		...
		pRenderingDeviceContext) override
	{
		...
		if (pRenderingDeviceContext->GetRenderingPipelineTable().TryBindShader(
			GetShaderAddress(), static_cast<UINT>(ShaderType::GEOMETRY_SHADER)))
		{
